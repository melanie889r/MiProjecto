	...
	public partial class Actor {
		...
		public void GetOverlappingCollisions() {
			...
			foreach (BoundingBoxProperties boundingBoxProperty in boundingBoxProperties) {
				...
				if (OverlapsGeometry) {
					Collider2D[] geometryHits = Physics2D.OverlapBoxAll(
						RealPosition + (Vector3) boundingBoxProperty.RealOffset,
						boundingBoxProperty.RealSize,
						0
					);

					for (int index = 0; index < geometryHits.Length; index++) {
						Collider2D hit = geometryHits[index];
				...
				if (OverlapsOtherActors) {
					Collider2D[] actorHits = Physics2D.OverlapBoxAll(
						RealPosition + (Vector3) boundingBoxProperty.RealOffset,
						boundingBoxProperty.RealSize,
						0,
					...
					for (int index = 0; index < actorHits.Length; index++) {
						Collider2D hit = actorHits[index];
						if (!hit || hit.gameObject == GameObject) {
							continue;
						}
				...
				if (OverlapsWeapons) {
					Collider2D[] weaponHits = Physics2D.OverlapBoxAll(
						RealPosition + (Vector3) boundingBoxProperty.RealOffset,
						boundingBoxProperty.RealSize,
						0,
					...
					for (int index = 0; index < weaponHits.Length; index++) {
						Collider2D hit = weaponHits[index];
						if (!hit || hit.gameObject == GameObject) {
							continue;
						}
